      ...
      <div style={{
        ...
        <div style={{ paddingBottom: 36, marginBottom: 32, borderBottom: `1px solid ${C.border}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 20 }}>
            ...
            {itin.dataConfidence && (() => {
              const dc = itin.dataConfidence
              const styles: Record<string, { bg: string; border: string; color: string }> = {
                high:   { bg: 'rgba(52,212,117,0.09)',   border: 'rgba(52,212,117,0.28)',  color: '#34d475' },
                medium: { bg: 'rgba(234,179,8,0.10)',    border: 'rgba(234,179,8,0.30)',   color: '#eab308' },
                low:    { bg: 'rgba(249,115,22,0.10)',   border: 'rgba(249,115,22,0.30)',  color: '#f97316' },
              }
              const s = styles[dc.flag]
              const label = dc.flag === 'high'
                ? '✓ Verified with live data'
                : dc.flag === 'medium'
                ? '~ Partially verified with live data'
                : '⚠ Some suggestions may be AI-generated'
              const tooltip = dc.flag === 'low'
                ? 'Place names could not all be verified against live data sources'
                : undefined
